...
// 実際の埋め込み関数のモック
function createRealisticEmbedder() {
  return vi.fn(async (text: string) => {
    ...
    const keywords = {
      ...
      "収益化": [0, 0, 0, 0, 1],
      "収益": [0, 0, 0, 0, 1],
      "プログラミング": [0, 0, 0, 0, 0], // 資料にない内容
    };

    // テキスト内のキーワードを検索して最も関連性の高い埋め込みを返す
    for (const [keyword, embedding] of Object.entries(keywords)) {
...
describe("Instagram RAG Integration Tests", () => {
  describe("基本的な検索機能", () => {
    it("バズの定義について正しく回答できる", async () => {
      const embed = createRealisticEmbedder();
      const loadAll = vi.fn(async () => instagramChunks);
      const generate = vi.fn(async (prompt: string) => {
        // プロンプトにバズの内容が含まれていることを確認
