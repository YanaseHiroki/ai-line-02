...
function createMockEmbedder() {
  return vi.fn(async (text: string) => {
    ...
    if (text.includes("バズ")) return [1, 0, 0, 0, 0];
    if (text.includes("アルゴリズム")) return [0, 1, 0, 0, 0];
    if (text.includes("投稿時間") || text.includes("ベストタイム")) return [0, 0, 1, 0, 0];
    if (text.includes("投稿頻度") || text.includes("頻度")) return [0, 0, 0, 1, 0];
    if (text.includes("ハッシュタグ")) return [0, 0, 0, 0, 1];
    if (text.includes("写真") || text.includes("動画")) return [0, 0, 0, 0, 1];
    if (text.includes("キャプション")) return [0, 0, 0, 0, 1];
    if (text.includes("エンゲージメント")) return [0, 0, 0, 0, 1];
    if (text.includes("リール")) return [0, 0, 0, 0, 1];
    if (text.includes("プロフィール")) return [0, 0, 0, 0, 1];
    if (text.includes("分析") || text.includes("指標")) return [0, 0, 0, 0, 1];
    if (text.includes("ブランディング") || text.includes("ブランド")) return [0, 0, 0, 0, 1];
    if (text.includes("収益化") || text.includes("収益")) return [0, 0, 0, 0, 1];
    if (text.includes("プログラミング")) return [0, 0, 0, 0, 0]; // 資料にない内容
    return [0, 0, 0, 0, 0]; // デフォルト
  });
}
